...
for i in range(num_iterations):
    ...
    for j in range(len(posts['posts'])):
        ...
        tags = [ y.strip() for x in posts['posts'][j]['tags'] for y in x.split('\n')  ]
        url = posts['posts'][j]['photos'][0]['original_size']['url']
        #if this is slow, switch to batch execute instead
        tag_ids = ts_model.add_tags(c, tags)
        photo_id = ts_model.add_photo(c, url, note_count)
        ts_model.link_tags_photo(c, tag_ids, photo_id)
    conn.commit()
conn.close()
